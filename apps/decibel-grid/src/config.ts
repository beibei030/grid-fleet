...
export const config = {
  ...
  /** extended-grid 独立进程（仅 localhost 聚合，写单仍走 8081+代理） */
  gridFleet: {
    url: str("GRID_FLEET_URL", "http://127.0.0.1:8081"),
    token: str("GRID_FLEET_TOKEN") || resolveAuthToken(),
  },

  /** decibel-grid 独立进程（:8083，与 8787 隔离） */
  decGridFleet: {
    url: str("DEC_GRID_FLEET_URL", "http://127.0.0.1:8083"),
    token: str("DEC_GRID_FLEET_TOKEN") || resolveAuthToken(),
  },

  /** ondo-grid 独立进程（:8084，与 8787 隔离） */
  ondoGridFleet: {
    url: str("ONDO_GRID_FLEET_URL", "http://127.0.0.1:8084"),
    token: str("ONDO_GRID_FLEET_TOKEN") || resolveAuthToken(),
  },

  /** risex-grid 独立进程（:8082） */
  risexGridFleet: {
    url: str("RISEX_GRID_FLEET_URL", "http://127.0.0.1:8082"),
    token: str("RISEX_GRID_FLEET_TOKEN") || resolveAuthToken(),
  },

  /** 四所网格总看板（:8088，只读聚合） */
  overviewPort: num("OVERVIEW_PORT", 8088),
