...
function emptyData() {
  return {
    ...
    baselineEquity: null,
    baselineSetAt: null,
    /** 重配/重置时官网 totalPnl 快照，用于本轮 = 当前官网累计 − 基准 */
    baselineOfficialTotalPnl: null,
    baselineOfficialRealizedPnl: null,
    sessionStartAt: null,
...
export class GridJournal {
  ...
  resetSession(equity, official = null) {
    ...
    this.data.fills = [];
    this.data.alerts = [];
    this.data.seenFillIds = [];
    if (eq > 0) {
      this.data.baselineEquity = eq;
      this.data.baselineSetAt = now;
    }
    const offTotal = official?.totalPnl;
    const offReal = official?.realizedPnl;
    if (offTotal != null && Number.isFinite(Number(offTotal))) {
      this.data.baselineOfficialTotalPnl = round2(offTotal);
      this.data.baselineOfficialRealizedPnl = offReal != null ? round2(offReal) : null;
    } else {
      this.data.baselineOfficialTotalPnl = 0;
      this.data.baselineOfficialRealizedPnl = 0;
    }
    this._saveNow();
    return {
      sessionStartAt: now,
      baselineEquity: this.getBaselineEquity(),
      baselineOfficialTotalPnl: this.getBaselineOfficialTotalPnl(),
    };
  }
  ...
  /** 仅 session/reset 时调用：锁定官网盈亏起点，此后本轮 = 官网累计 − 此值 */
  setOfficialBaseline(official) {
    if (official?.totalPnl == null) return null;
    this.data.baselineOfficialTotalPnl = round2(official.totalPnl);
    this.data.baselineOfficialRealizedPnl = official.realizedPnl != null
      ? round2(official.realizedPnl)
      : null;
    this._scheduleSave();
    return this.data.baselineOfficialTotalPnl;
  }

  getBaselineOfficialTotalPnl() {
    return this.data.baselineOfficialTotalPnl != null
      ? Number(this.data.baselineOfficialTotalPnl)
      : null;
  }

  getBaselineOfficialRealizedPnl() {
    return this.data.baselineOfficialRealizedPnl != null
      ? Number(this.data.baselineOfficialRealizedPnl)
      : null;
  }
